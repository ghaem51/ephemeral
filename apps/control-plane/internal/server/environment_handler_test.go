...
func TestCreateEnvironment(t *testing.T) {
	...
	service.create = func(_ context.Context, request createenvironment.Request) (*environmentapi.Result, error) {
		if request.Name != "feature-payment" || request.Image != "envpilot/demo-service:healthy" || request.ContainerPort != 8080 || request.HealthCheckPath != "/ready" || len(request.EnvironmentVariables) != 1 || request.EnvironmentVariables[0] != "LOG_LEVEL=debug" {
	...
	body := `{"name":"feature-payment","image":"envpilot/demo-service:healthy","containerPort":8080,"healthCheckPath":"/ready","environmentVariables":["LOG_LEVEL=debug"],"simulateFailure":true}`
